...
(function () {
  ...
  function ensureStyle() {
    ...
    var css = [
      ...
      ".epub-toolbar button:disabled{opacity:.45;cursor:default;}",
  ...
  function setPagingEnabled(paginated) {
    var dis = !paginated;
    if (btnPrev) btnPrev.disabled = dis;
    if (btnNext) btnNext.disabled = dis;
  }
  ...
  function rebuildRendition() {
    ...
    setPagingEnabled(flow === "paginated");
  ...
  function mount(host, info) {
    ...
    // 模式切换
    var btnMode = el("button", null, "翻页模式");
    ...
    // 目录
    var tocSel = document.createElement("select");
    tocSel.title = "目录";
    ...
    setPagingEnabled(false); // 默认滚动模式，翻页按钮禁用

# 构建 Loom 桌面版 exe（独立原生窗口程序，目标机无需安装 Python）
#
...
#
# 产物：dist\Loom.exe —— 双击即开一个原生窗口（无浏览器外壳）。
...
Get-Process Loom -ErrorAction SilentlyContinue | Stop-Process -Force
for ($i = 0; $i -lt 20 -and (Get-Process Loom -ErrorAction SilentlyContinue); $i++) { Start-Sleep -Milliseconds 300 }
Start-Sleep -Milliseconds 800

# 先删旧产物：让“构建后 exe 是否存在”真实反映本次结果，而不是被上一次的旧文件骗过
Remove-Item dist\Loom.exe -Force -ErrorAction SilentlyContinue
...
# --add-data "docs;docs"：打包轻量生态路线等只读项目资料
# 入口 desktop.py 会 import server，PyInstaller 自动一并打包
pyinstaller --noconfirm --onefile --name Loom --add-data "static;static" --add-data ".workbench;.workbench" --add-data "docs;docs" --collect-all winpty --icon icon.ico --windowed desktop.py
$pyExit = $LASTEXITCODE

# 因为上面已先删旧 exe，这里 Test-Path 为真即代表“本次确实新生成了 exe”，不再是假阳性
if ($pyExit -eq 0 -and (Test-Path dist\Loom.exe)) {
    $mb = [math]::Round((Get-Item dist\Loom.exe).Length / 1MB, 1)
    Write-Host ""
    Write-Host "[OK] 构建完成: dist\Loom.exe ($mb MB)  @ $((Get-Item dist\Loom.exe).LastWriteTime)" -ForegroundColor Green
    Write-Host "     双击运行 → 弹出原生窗口；默认恢复上次工作区，无历史时显示欢迎页"
    Write-Host "     指定工作区: Loom.exe D:\notes"
} else {
    Write-Host "[FAIL] 构建失败 (PyInstaller exit=$pyExit)，未生成新 exe。" -ForegroundColor Red
    Write-Host "       最常见原因: 仍有 Loom 实例占用文件锁未释放, 确认进程已退出后重试。" -ForegroundColor Red
    exit 1
}
